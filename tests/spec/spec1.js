describe('Javascipt learnings', function () {
	describe('Functions to fix', function () {
		describe('addTwo', function () {
			it('adds two numbers', ()=>{
				expect(addTwo(1,1)).toEqual(2);
			})
		});
		describe('combineTwoArrays', function () {
			it('combines two arrays', () => {
				expect(combineTwoArrays([1,2], ['a', 'b'])).toEqual([1,2,'a','b']);
			})
		})
	...
	describe('Functions to make', function () {
		describe('returnsBiggestNumber', () => {
			it('returns a Number', () => {
				expect(returnsBiggestNumber(1, 2)).toEqual(jasmine.any(Number));
			});
			it('returns the BIGGEST NUMBER from two', () => {
				expect(returnsBiggestNumber(1, 2)).toEqual(2);
			});
		})
		describe('returnsBiggestWord', ()=>{
			it('returns a String', ()=>{
				expect(returnsBigWord(['sss', 'ssssss', 'sfsfsfsfsf'])).toEqual(jasmine.any(String));
			});
			it('returns the BIGGEST Length Word from two', () => {
				expect(returnsBigWord(['sss', 'ssssss', 'sfsfsfsfsf'])).toEqual('sfsfsfsfsf');
				expect(returnsBigWord(['sss', 'sfsfsfsfsf', 'ssssss', ''])).toEqual('sfsfsfsfsf');
			});
		})
		describe('isItAnEvenNumber', () => {
			it('returns a Boolean', () => {
				expect(isItAnEvenNumber(2)).toEqual(jasmine.any(Boolean));
			});
			it('returns true if the number is even', () => {
				expect(isItAnEvenNumber(2)).toEqual(true);
			});
			it('returns false if the number is odd', () => {
				expect(isItAnEvenNumber(3)).toEqual(false);
			});
		})
		describe('returnsBigWord', () => {
			it('returns a String', () => {
				expect(returnsBigWord(['as', 'aaaddd', 'dddsdfsd'])).toEqual(jasmine.any(String));
			});
			it('returns the LONGEST string', () => {
				expect(returnsBigWord(['as', 'aaaddd', 'dddsdfsd'])).toEqual('dddsdfsd');
				expect(returnsBigWord(['aaaddd', 'dddsdfsd', 'as'])).toEqual('dddsdfsd');
			});
		})
	});
})
